...
quizzes <- readLines('bench_data/quizzes.txt', encoding = 'UTF-8')
...
tweets <- readLines('bench_data/tweets.txt', encoding = 'UTF-8')
...
blogs <- readLines('bench_data/blogs.txt', encoding = 'UTF-8')
           ...
           function(list.name) {  
               ...
               time <- system.time({
                   for (sent in sentences) {
                       print(paste0(list.name,"-",which(sent==sentences)))
                       ...
                       rank <- sapply(seq_len(ncol(split)),
                                      function(i) {
                                          min(which(predict.baseline(split[1,i])==split[2,i]),4)
                                      })
...
predict.baseline <- function(x){Prediction(x)}
